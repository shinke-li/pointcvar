...
import util.pytorch_utils as pytorch_utils
...
class JGBA:
    def __init__(self, model, eps=0.1, n=40, eps_iter=0.01) -> None:
        self.model = model.cuda()
        self.model.eval()
        
        self.eps = float(eps)
        self.eps_iter = float(eps_iter)
        self.n = int(n)
    def attack(self, data, target):
        ...
        if len(x.shape) == 3:
            x = x[0]
        batch_size = x.size()[0]
        x_advs = x.detach().cpu().numpy()
        yvars = y.clone() #pytorch_utils.to_var(torch.LongTensor([y]), cuda=True)
        x_perturb = []
        for b in range(batch_size):
            ...
            for _ in range(n):
                ...
                outputs_should = self.model(xvar_should)
            ...
            x_perturb.append(np.copy(x_adv))
        x_perturb = np.asarray(x_perturb)
        with torch.no_grad():
            logits = self.model(torch.from_numpy(x_perturb).to(self.model))
            ...
            success_num = (pred == target).\
                sum().detach().cpu().item()
        print('Final success: {}/{}'.
              format(success_num, batch_size))
        
        return x_perturb, success_num
