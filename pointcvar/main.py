...
def check_inp_fmt(task, data_batch, dataset_name):
    if task in ['cls', 'cls_trans']:
        ...
        if dataset_name == 'modelnet40_dgcnn' or dataset_name == 'shapenetpart_dgcnn':
            pc = torch.FloatTensor(pc)
            assert isinstance(pc, torch.Tensor) or isinstance(
...
def get_loss(task, loss_name, data_batch, out, dataset_name,*args, **kwargs):
...
def validate_robust(task, 
    ...
    if confusion:
        pred = []
        ground = []
    myground = np.zeros(40)
    for i, data_batch in enumerate(tqdm(loader, total=len(loader))):
        time1 = time()
        if method=='cvar':
            data_batch = aug_utils.advanced_cvar(data_batch, model, task,  dataset_name=dataset_name, **method_kwargs)
            if aug_utils.RETURN_RISK: 
                f = './iter_num/ptbd2_mu50' + '.npz'
                pc,label,risk = data_batch['pc'].cpu().detach().numpy(), data_batch['label'].cpu().detach().numpy(), data_batch['risk']
                if os.path.exists(f):
                    dgcnn_data = np.load(f)
                    pc1 = dgcnn_data['pc']
                    label1 = dgcnn_data['label']
                    risk1 = dgcnn_data['risk']
                    new_pc = np.concatenate((pc1, pc), axis=0)
                    new_label = np.concatenate((label1, label), axis=0)
                    new_risk = np.concatenate((risk1, risk), axis=0)
                    np.savez(f, pc = new_pc, 
                            label = new_label,
                            risk = new_risk)
                else:
                    np.savez(f, pc = pc, label = label, risk = risk)
        elif method == 'dup':
            data_batch = defend_npz.defend(data_batch, method, cmd_args, **method_kwargs) 
            if aug_utils.RETURN_RISK: 
                f = './iter_num/dgcnn_dup_bd' + '.npz'
                pc,label = data_batch['pc'].cpu().detach().numpy(), data_batch['label'].cpu().detach().numpy()
                if os.path.exists(f):
                    dgcnn_data = np.load(f)
                    pc1 = dgcnn_data['pc']
                    label1 = dgcnn_data['label']
                    new_pc = np.concatenate((pc1, pc))
                    new_label = np.concatenate((label1, label))
                    np.savez(f, pc = new_pc, 
                            label = new_label,
                            )
                else:
                    np.savez(f, pc = pc, label = label)
        elif method == 'sor':
            data_batch = defend_npz.defend(data_batch, method, cmd_args, **method_kwargs) 
            if aug_utils.RETURN_RISK: 
                f = './iter_num/dgcnn_sor_bd' + '.npz'
                pc,label = data_batch['pc'].cpu().detach().numpy(), data_batch['label'].cpu().detach().numpy()
                if os.path.exists(f):
                    dgcnn_data = np.load(f)
                    pc1 = dgcnn_data['pc']
                    label1 = dgcnn_data['label']
                    new_pc = np.zeros((len(pc1[:])+len(pc[:]),max(len(pc1[0][:]),len(pc[0][:])), 3))#max(len(pc1[0][:]),len(pc[0][:]))
                    new_pc[:len(pc1),:len(pc1[0][:]),:] = pc1
                    new_pc[len(pc1):,:len(pc[0][:]),:] = pc
                    new_label = np.concatenate((label1, label),axis=0)
                    np.savez(f, pc = new_pc, 
                            label = new_label,
                            )
                    ...
                    np.savez(f, pc = pc, label = label)            
        elif method in ['srs', 'ror']:
            data_batch = defend_npz.defend(data_batch, method, cmd_args, **method_kwargs) 
        elif method == 'dup_cvar':
            data_batch = aug_utils.advanced_cvar(data_batch, model, task,  dataset_name=dataset_name, **method_kwargs)
            data_batch = defend_npz.defend(data_batch, 'dup', cmd_args, **method_kwargs) 
        elif method == 'dup_itercvar':
            data_batch = aug_utils.advanced_cvar(data_batch, model, task,  dataset_name=dataset_name, **method_kwargs)
            data_batch = defend_npz.defend(data_batch, 'dup', cmd_args, **method_kwargs) 
        ...
        if confusion:
            ...
            ground.append(data_batch['label'].squeeze().cpu())
        time3 = time()
        myground += np.bincount(data_batch['label'].squeeze().cpu().numpy(), minlength=40)
        perf.update(data_batch=data_batch, out=out)
        time4 = time()
        time_dl += (time1 - time5)
        time_gi += (time2 - time1)
...
def train(task, loader, model, optimizer, loss_name, dataset_name, cfg, epoch=None):
    model.train()
    def get_extra_param():
       return None
...
def entry_train(cfg, resume=False, model_path=""): 
    ...
    for epoch in range(cfg.TRAIN.num_epochs):
        ...
        print('Training..')
        train_perf, train_loss = train(cfg.EXP.TASK, loader_train, model, optimizer, cfg.EXP.LOSS_NAME, cfg.EXP.DATASET, cfg, epoch)
...
def entry_test(cfg, test_or_valid, model_path="", confusion = False):
    ...
    if cfg.ROBUSTINFER.ACTIVATE : 
        method = cfg.ROBUSTINFER.METHOD.upper()
        params = dict(getattr(cfg.ROBUSTINFER.PARAMS, method))
        test_perf = validate_robust(cfg.EXP.TASK, loader_test, model, cfg.EXP.DATASET, cfg.ADAPT, confusion, cfg.ROBUSTINFER.METHOD, params)

    else:
        test_perf = validate(cfg.EXP.TASK, loader_test, model, cfg.EXP.DATASET, cfg.ADAPT, confusion)
        
    if confusion:
        test_perf, pred, ground  = test_perf[0], test_perf[1], test_perf[2]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ror_nb', type=int, default=4,
                        help='number of neighbor')
    parser.add_argument('--ror_radius', type=float, default=0.1,
                        help='raidus of removal')
